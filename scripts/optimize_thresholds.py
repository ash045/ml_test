...
def _ensure_ret_col(df: pd.DataFrame, cfg) -> str:
    if "ev_ret" in df.columns:
        return "ev_ret"
    ret_col = cfg.get("execution", {}).get("ret_col")
...
def _equity_and_dd(returns: np.ndarray):
...
def _maybe_apply_mapper(df: pd.DataFrame, cfg: dict, base_action: np.ndarray):
    ...
    try:
        ...
        # diagnostics
        diag = {
            "base":   {"long": int((base_action == 1).sum()), "short": int((base_action == -1).sum()),
                       "trades": int((base_action != 0).sum())},
            "mapper": {"long": int((mapped == 1).sum()), "short": int((mapped == -1).sum()),
                       "trades": int((mapped != 0).sum())},
            "combo":  {"long": int((combined == 1).sum()), "short": int((combined == -1).sum()),
...
def _evaluate_thresholds(data: pd.DataFrame, ret_col: str, fee_bps: float, spread_bps: float,
                         t_long: float, t_short: float, apply_mapping: bool, cfg: dict,
...
def _grid_search(train_df: pd.DataFrame, ret_col: str, fee_bps: float, spread_bps: float,
                 apply_mapping: bool, cfg: dict, min_trades_train: int):
    ...
    for tL in longs:
        for tS in shorts:
            if not (tS < 0.5 < tL):
                continue
            res = _evaluate_thresholds(train_df, ret_col, fee_bps, spread_bps,
                                       float(tL), float(tS), apply_mapping, cfg, return_equity=False)
...
def _plot_heatmap(train_table: pd.DataFrame, path_png: str):
    ...
    plt.xlabel("t_long"); plt.ylabel("t_short")
...
def _plot_equity(eq: np.ndarray, path_png: str, title: str):
    ...
    plt.plot(eq, lw=1.5)
    plt.xlabel("Events (chronological)"); plt.ylabel("Cumulative return")
    plt.title(title)
    plt.tight_layout(); plt.savefig(path_png, dpi=120); plt.close()
...
def main(cfg_path: str, timeframe: str = "1min"):
    ...
    # Merge on timestamp (keep rows with OOF)
    ...
    # Choose realized-return column
    ret_col = _ensure_ret_col(d, cfg)
    d[ret_col] = pd.to_numeric(d[ret_col], errors="coerce")
    d = d.dropna(subset=[ret_col]).reset_index(drop=True)

    if len(d) < 200:
        log.warning(f"Only {len(d)} OOF events available after merge/clean. Results may be unstable.")
    ...
    frac = float(holdout_cfg.get("holdout_frac", 0.8));  frac = min(max(frac, 0.5), 0.95)
    ...
    n = len(d)
    cut = max(int(n * frac), n - 200); cut = min(max(cut, 100), n - 50)
    train_df = d.iloc[:cut].copy(); hold_df  = d.iloc[cut:].copy()

    log.info(f"Threshold search on TRAIN slice: n={len(train_df)} | HOLDOUT: n={len(hold_df)} | apply_mapping={apply_mapping}")

    # --- Search on TRAIN ---
    train_table, best_train = _grid_search(train_df, ret_col, fee_bps, spread_bps, apply_mapping, cfg, min_tr_train)
    ...
    if best_train is None or train_table.empty:
        ...
        with open(f"{base}_best.json", "w", encoding="utf-8") as f:
            json.dump({"error": "no_thresholds_passed_min_trade_constraint",
                       "min_trades_train": min_tr_train}, f, indent=2)
        log.warning(f"No thresholds met min_trades_train={min_tr_train}.")
        return

    # Evaluate best TRAIN thresholds on HOLDOUT (with diagnostics)
    best_on_hold = _evaluate_thresholds(
        hold_df, ret_col, fee_bps, spread_bps,
        best_train["t_long"], best_train["t_short"],
        apply_mapping, cfg, return_equity=True
    )
    ...
    for i in range(topK):
        ...
        res = _evaluate_thresholds(
            hold_df, ret_col, fee_bps, spread_bps,
            row["t_long"], row["t_short"], apply_mapping, cfg, return_equity=False
        )
    ...
    grid_csv = f"{base}_grid_train.csv"; train_table.to_csv(grid_csv, index=False)
    heatmap_png = f"{base}_heatmap_train.png"; 
    _plot_heatmap(train_table, heatmap_png)
    top10_csv = f"{base}_top10_train.csv"; train_table.head(10).to_csv(top10_csv, index=False)
    ...
    def _fmt_diag(dg):
        if not dg: return "n/a"
        b = dg["base"]; m = dg["mapper"]; c = dg["combo"]
        return (f"base L/S/T={b['long']}/{b['short']}/{b['trades']} | "
    ...
    def _flt(v): return float(v) if isinstance(v, (np.floating, np.integer)) else v
    summary = {
        "ret_col": ret_col,
        "holdout_frac": frac,
        ...
        "holdout_at_train_best": {k: _flt(v) for k, v in best_on_hold.items() if k != "equity"},
        "best_on_holdout_among_top_train": ({k: _flt(v) for k, v in best_holdout.items()} if best_holdout else None),
    ...
    log.info(f"Best TRAIN thresholds: t_long={best_train['t_long']:.3f}, t_short={best_train['t_short']:.3f}, "
    ...
    log.info(f"HOLDOUT @ train-best: trades={best_on_hold['trades']}, "
    ...
    if best_holdout:
        log.info(f"Best-on-HOLDOUT among top {min(50, len(train_table))}: "
    ...
    log.info(f"Artifacts → grid:{grid_csv} | heatmap:{heatmap_png} | top10:{top10_csv} | equity (holdout):{eq_png}")
